...
public class RecipeServiceImple implements RecipeService {
	...
	public List<RecipeDetail> getRecipeByUserId(Integer userId) {
		...
		List<RecipeDetail> recipes=recipeDao.getRecipeByUserId(userId);
		if(recipes.isEmpty()) {
			throw new RuntimeException("you Did Not Have Any Recipe");
		}
		return recipes;
	}

	@Override
	public List<RecipeDetail> userRecipes(Integer userId) {
		
		List<RecipeDetail> userrecipes=recipeDao.getRecipeByUserId(userId);
		return userrecipes;
	}
